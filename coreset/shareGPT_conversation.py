...
def add_parser_arg(parser):
    ...
    parser.add_argument('--output', type=str, nargs="?",
                        ...
                        default="/data/benchmarksuite/sharegpt/sharegpt_conversation.json")
...
if __name__ == "__main__":
            ...
            if data['conversations'][message_idx]["from"] == "human":
                try:
                    if data['conversations'][message_idx+1]["from"] == "gpt":
                        prefix_prompt += data['conversations'][message_idx]["value"]
                        prompt.append(prefix_prompt)
                except:
                    continue
                ...
                try:
                    if data['conversations'][message_idx_previous]["from"] == "human":
                        prefix_prompt += data['conversations'][message_idx]["value"]
                        answer.append(data['conversations'][message_idx]["value"])
                except:
                    continue
    ...
    for i in range(len(prompts)):
        assert len(answers[i]) == len(prompts[i])
